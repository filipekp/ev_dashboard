<?php
  ...
  <footer class="site-footer">created by: &copy; 2026 Pavel Filípek (<a href="https://www.filipek-czech.cz" target="_blank" rel="noopener noreferrer">www.filipek-czech.cz</a>)</footer>
  </body></html><?php exit;
  }
  ...
  $bandLabels = ['Město (<40 km/h)', 'Okresky (40–65 km/h)', 'Rychlé okresky (65–85 km/h)', 'Dálnice (>85 km/h)'];
  ...
  foreach ($routeQ->fetchAll() as $r) {
    $routes[displayRoute($r['start_address'], $r['end_address'])] = [
      'count' => (int)$r['c'], 'km' => (float)$r['km'], 'kwh' => (float)$r['kwh']
    ];
  }
...
<header class="topbar">
  ...
  <form class="vehicle-switch" method="get"><select name="vehicle_id" onchange="this.form.submit()"
        ...
        <option value="<?= $v['id'] ?>" <?= (int)$v['id'] === (int)$vehicle['id'] ? 'selected' : '' ?>><?= (int)($user['default_vehicle_id'] ?? 0) === (int)$v['id'] ? '★ ' : '' ?><?= h($v['name']) ?>
        · <?= h($v['vin']) ?></option><?php endforeach; ?></select></form>
  <div class="vehicle">
...
<main class="wrap">
  ...
  <section class="period-filter">
    <div class="period-main">
      <b>🗓 Období:</b>
      <a class="period-all <?= $period === 'all' ? 'active' : '' ?>" href="?vehicle_id=<?= $vehicle['id'] ?>&amp;period=all&amp;year=<?= h($selectedYear) ?>">Celá historie</a>
      <form method="get" class="year-filter">
        ...
        <select id="periodYear" name="year" onchange="this.form.submit()">
          <?php foreach ($years as $y): ?><option value="<?= h($y) ?>" <?= $selectedYear === $y ? 'selected' : '' ?>><?= h($y) ?></option><?php endforeach; ?>
        </select>
      </form>
    </div>
  ...
  <section class="grid2">
    ...
    <div class="card energy"><h2>⚡ Energetická bilance a nabíjecí lokality</h2><?php if ($chargeDataAvailable): ?><p>Odhad energie podle veřejného
        ...
        <footer>Celkem v bilanci: <b><?= cz($chargeTotal, 0) ?> kWh</b><span>* veřejná energie je odhad z přírůstku SoC</span></footer><?php else: ?>
      ...
      <p>Nejčastější směry v importovaných datech</p><?php if ($routes): ?>
        ...
        foreach ($routes as $name => $r) {
          ...
          <div><b><?= h($name) ?></b><span><?= $r['count'] ?>×</span><strong><?= cz($c, 1) ?> <em>kWh/100 km</em></strong><small>Průměrná
            délka: <?= cz($r['km'] / $r['count'], 1) ?> km</small></div><?php } ?></div><?php else: ?><p><b>Trasy nejsou v tomto CSV k dispozici.</b>
    ...
    <div class="card"><h2>📍 Nejčastější pravidelné trasy</h2>
      <p>Statistika tras s nejvyšším počtem opakování</p><?php if ($routes): ?>
        ...
        foreach ($routes as $name => $r) {
          ...
          <small><?= $r['count'] ?>× jízda</small></div><?php } ?></div><?php else: ?><p><b>Trasy nejsou dostupné.</b> Export Citigo iV neobsahuje GPS
  ...
  <section class="card table-card history-card">
    <div class="table-card-head">
      <div><h2>📋 Seznam a historie jízd</h2><p>Jízdy v aktuálním filtru · stránka <?= $historyPage ?> z <?= $historyPages ?></p></div>
    ...
    <?php if ($historyPages > 1): ?>
      <nav class="pagination" aria-label="Stránkování historie jízd">
        <?php if ($historyPage > 1): ?><a href="?vehicle_id=<?= $vehicle['id'] ?>&amp;period=<?= h($period) ?>&amp;year=<?= h($selectedYear) ?>&amp;page=<?= $historyPage - 1 ?>">← Předchozí</a><?php endif; ?>
        <span>Stránka <b><?= $historyPage ?></b> / <?= $historyPages ?> · <?= $tripCount ?> jízd</span>
        <?php if ($historyPage < $historyPages): ?><a href="?vehicle_id=<?= $vehicle['id'] ?>&amp;period=<?= h($period) ?>&amp;year=<?= h($selectedYear) ?>&amp;page=<?= $historyPage + 1 ?>">Další →</a><?php endif; ?>
      </nav>
    <?php endif; ?>
  </section>
</main>
...
<script>
  ...
  document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && !mobileMenuOverlay.hidden) closeMobileMenu();
  });
  ...
  <?php if($chargeDataAvailable):?>new Chart(document.getElementById('energy'), {
    type   : 'doughnut',
    data   : {
      labels  : ['AC / ostatní', 'Veřejné DC'],
      datasets: [{ data: [<?=round($homeKwh, 2)?>, <?=round($publicKwh, 2)?>], backgroundColor: ['#10b981', '#f59e0b'], borderWidth: 0 }]
    },
