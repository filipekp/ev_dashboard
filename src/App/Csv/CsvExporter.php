...
/**
     * Exports normalized trip data through the appropriate CSV plugin.
     *
...
final class CsvExporter
{
    ...
    private function period(string $period, string $year): array
    {
        if (preg_match('/^\d{4}-\d{2}$/', $period)) {
            ...
            $d = new \DateTime($from);
            $d->modify('+1 month');
            return [$period, $from, $d->format('Y-m-d H:i:s'), '_' . $period];
        }
        if ($period === 'year' && preg_match('/^\d{4}$/', $year)) {
            return [$period, $year . '-01-01 00:00:00', ((int)$year + 1) . '-01-01 00:00:00', '_' . $year];
        }
