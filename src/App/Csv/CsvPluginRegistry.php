...
final class CsvPluginRegistry
{
    ...
    public function all(): array { return array_values($this->plugins); }

    /** @param string[] $header */
    public function detect(array $header): CsvVehiclePluginInterface
    {
        foreach ($this->plugins as $plugin) {
            if ($plugin->supports($header)) return $plugin;
        }
    ...
    {
        if (!isset($this->plugins[$id])) throw new RuntimeException('CSV plugin "' . $id . '" není registrován.');
        return $this->plugins[$id];
    }

    public function has(string $id): bool { return isset($this->plugins[$id]); }
}
