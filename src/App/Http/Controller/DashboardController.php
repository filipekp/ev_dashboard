...
/**
     * Coordinates dashboard requests and vehicle dashboard actions.
     *
...
final class DashboardController
{
    ...
    public function handle(): void
    {
        ...
        if (!$vehicle) {
            $this->app->template()->render('dashboard-empty', ['app' => $this->app, 'user' => $user, 'flash' => $flash, 'vehicles' => $vehicles]);
            return;
        }
    ...
    private function completeNewVehicle(array $user): void
    {
        try {
            ...
            $q = $this->app->pdo()->prepare('UPDATE vehicles SET name=?,battery_kwh=?,battery_nominal_kwh=?,soh_manual_pct=?,soh_manual_at=IF(? IS NULL,NULL,NOW()),home_label=? WHERE id=?');
            $q->execute([$name, $battery, $nominal, $soh, $soh, $home?:NULL, $vehicleId]);
            ...
            Http::redirect('index.php' . ($vid > 0?'?vehicle_id=' . $vid . '&new_vehicle=1':''));
        }
    }
}
