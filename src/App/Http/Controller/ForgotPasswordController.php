    ...
    public function handle(): void
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                ...
                $q = $this->app->pdo()->prepare('SELECT id,name,email,active FROM users WHERE email=? LIMIT 1');
                $q->execute([$email]);
                $u = $q->fetch();
                if ($u && (int)$u['active']) {
                    $token = $this->app->auth()->createPasswordResetToken((int)$u['id']);
                    $url = $this->app->auth()->resetUrl($token);
                    $this->app->auth()->sendPasswordResetEmail($u['email'], $u['name'], $url);
        ...
        $this->app->template()->render('forgot-password', ['app' => $this->app, 'message' => $message, 'debugUrl' => $debugUrl]);
    }
}
