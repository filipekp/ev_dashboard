...
/**
 * Manages vehicles and vehicle-to-user assignments.
 *
        ...
        try {
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                if ($action === 'create' || $action === 'update') {
                    ...
                    if ($action === 'create') {
                        $pdo->prepare('INSERT INTO vehicles(name,vin,battery_kwh,battery_nominal_kwh,soh_manual_pct,soh_manual_at,home_label) VALUES(?,?,?,?,?,IF(? IS NULL,NULL,NOW()),?)')->execute([
                            $name,
                            $vin,
                            $battery,
                            $nominal,
                        ...
                        $pdo->prepare('UPDATE vehicles SET name=?,vin=?,battery_kwh=?,battery_nominal_kwh=?,soh_manual_pct=?,soh_manual_at=IF(? IS NULL,NULL,NOW()),home_label=? WHERE id=?')->execute([
                            $name,
                            $vin,
                            $battery,
                            $nominal,
                    ...
                    foreach (array_unique($userIds) as $uid) {
                        if ($uid > 0) {
                            $ins->execute([$uid, $vehicleId]);
                        }
                    }
                    $pdo->prepare("UPDATE users u SET u.default_vehicle_id=NULL WHERE u.default_vehicle_id=? AND u.role='user' AND NOT EXISTS (SELECT 1 FROM user_vehicles uv WHERE uv.user_id=u.id AND uv.vehicle_id=?)")->execute([$vehicleId, $vehicleId]);
                    $pdo->commit();
                    ...
                    $pdo->prepare('DELETE FROM vehicles WHERE id=?')->execute([(int)$_POST['id']]);
                    $this->app->session()->flash('Vozidlo a jeho jízdy byly smazány.');
                }
