...
/**
     * Builds aggregated statistics and datasets for the dashboard.
     *
...
final class DashboardService
{
    ...
    public function build(array $vehicle, array $query): array
    {
        ...
        $summarySql =
            ...
            "MIN(end_soc) min_soc, " .
            "SUM(CASE WHEN start_soc IS NOT NULL OR end_soc IS NOT NULL " .
            "OR public_charging_stops>0 OR public_charge_soc_gained>0 THEN 1 ELSE 0 END) charge_rows, " .
        ...
        $summaryQ = $this->pdo->prepare($summarySql);
        ...
        foreach ($q->fetchAll() as $r) {
            ...
            $routes[$key] = ['count' => (int)$r['c'], 'km' => (float)$r['km'], 'kwh' => (float)$r['kwh']];
        }
