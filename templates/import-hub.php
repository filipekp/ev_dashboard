...
<article class="import-tile"><span>▤</span><h2>Faktura / účtenka</h2><p>PDF, obrázek nebo JSON. Nejdřív lokální parser Powerpass/Elli, ČEZ a E.ON; následně volitelně AI.</p><form action="documents.php" method="post" enctype="multipart/form-data"><input type="hidden" name="csrf" value="<?=h(csrfToken())?>"><input type="hidden" name="action" value="upload_document"><input type="file" name="document" accept=".pdf,.json,image/*" required><button class="btn primary">Vytěžit dokument</button></form></article>
